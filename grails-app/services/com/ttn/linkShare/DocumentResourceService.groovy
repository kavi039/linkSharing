...
class DocumentResourceService {
    ...
    def save(DocumentCO documentCO) {
        File newFile = null
        MultipartFile file = documentCO.document
        String name = file.getOriginalFilename()
        documentCO.topic = Topic.findByName(documentCO.name)
        documentCO.user = documentCO.topic.user
        if (name) {
        File    newFileDir = new File("/home/intelligrape/Project/linkShare/Document/${documentCO.user.username}")
            newFileDir.mkdirs()
