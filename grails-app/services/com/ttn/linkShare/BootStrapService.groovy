 ...
 List<User> createUsers() {
    ...
    List< Topic> createTopic(List<User> userList) {
        ...
        userList.each { User user ->
            5.times {
                topic = new  Topic(name: "${it}${user.id}Groovy", visibility: (it % 2 ? Visibility.PRIVATE : Visibility.PUBLIC), user: user.id)
        ...
        topicList.each {  Topic topic ->
            2.times {
                linkResource = new LinkResource(description: "linkResource is created  by user name${topic.user.username}", user: topic.user, topic: topic,
                        url: "http://www.contentquality.com/mynewtester/cynthia.exe?Url1=http://domain.tld/&rptmode=2", isRead:(it%2==0), score: 21)
                linkResource.save(failOnError: true)
                documentResource = new DocumentResource(description: "DocumentResource is createdby${topic.user.username}", user: topic.user, topic: topic, filePath: "/file/kavita/bora",isRead:(it%2==0),score:(i%2==0?21:0))
