...
<li><a href="${createLink(controller: 'user',action: 'user',params: [id:User.findByUsername(''+session['username']).id])}" title="user Profile"><img src="${createLink(controller: 'image',action: 'renderImage',params: [id:com.ttn.linkShare.User.findByUsername("${session['username']}").id])}" height="25px" width="25px"/></a></li>
<li>
