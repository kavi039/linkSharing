...
<li><a href="${createLink(controller: 'user',action: 'user',params: [id:User.findByUsername(''+session['username']).id])}" title="user Profile"><img src="${createLink(controller: 'image',action: 'renderImage',params: [id:com.ttn.linkShare.User.findByUsername("${session['username']}").id])}" height="50px" width="50px"/></a></li>
<li>
